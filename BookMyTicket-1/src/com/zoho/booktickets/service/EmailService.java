...
public class EmailService {
    ...
    public static void generatedCancelPDF(long bookingId) {
        Document document = new Document();
        try {
            PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream(Constant.EMAIL_PDF_SEND));
            document.open();
    ...
    public void sendPDFMail(String to) {
        ...
        try {
            String filename = Constant.EMAIL_PDF_SEND;
    ...
    public void sendOTPMail(String to, String generatedOTP) {
    ...
    public static void sendExpiryAlert(String to, String expiryDays, String resetLink) {
        ...
        try {
            ...
            Multipart mp = new MimeMultipart();
            MimeBodyPart htmlPart = new MimeBodyPart();
            htmlPart.setContent(htmlTemplateExpiry(expiryDays, resetLink), "text/html");
            mp.addBodyPart(htmlPart);
            msg.setContent(mp);
            Transport transport = session.getTransport("smtp");
            transport.connect("smtp.gmail.com", 465, username, password);
    ...
    public static String htmlTemplateExpiry(String expiryDays, String resetLink) {
        return new StringBuilder(
                ...
                .append("</h2>")
                .append("Please change your before password expiry!!")
                .append("<p>Click Here Reset Password: " + resetLink + "</p>")
                .append("<h3>Note: Above link will expired in 10 minutes!!</h3></div></div>").toString();
    }
