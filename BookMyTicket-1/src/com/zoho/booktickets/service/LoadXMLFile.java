...
public class LoadXMLFile extends HttpServlet {
    ...
    public static final String xmlFilePath = Constant.DB_CREATE_XML;
                    ...
                    if (nSubNode.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        if (eElement.getAttribute("name").equals(source)) {
                            for (int k = 0; k < eSubElement.getElementsByTagName("target").getLength(); k++) {
                                ...
                                builder = new StringBuilder("CREATE TABLE IF NOT EXISTS " + target + " (LIKE " + source
                                        + " INCLUDING ALL);");
    ...
    public void init() throws ServletException {
        ...
        try {
            ...
            URL url = new URL(Constant.PORT_URL);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            System.out.println(conn.getResponseCode());
            System.out.println(queries);
            sendlinkTable();
            new UserService().checkMovieOrAdd(new User("admin", "Santheesh",
            "7904188021", "santheesh62@gmail.com", "Male"));
            new PortService().post(Constant.PORT_URL+"/dbcheck",
            JsonUtil.objectToString(queries));
        } catch(Exception e){
            System.out.println("Port:"+Constant.PORT_URL+" is not Active | Connection Failed");
            System.out.println(e.getMessage());
        }
    }
}